...
* [Reviewing Pull Requests](#reviewing-pull-requests)
  ...
  * [Accept that there are different opinions about what belongs in Al-Wasl.Connect](#accept-that-there-are-different-opinions-about-what-belongs-in-Al-Wasl.Connect)
...
More details to come on Al-Wasl.Connect dependencies. In case of doubt, open an issue in the
[issue tracker](https://github.com/Call-for-Code/Al-Wasl.Connect/issues) or engage on the 
...
Fork the project [on GitHub](https://github.com/Call-for-Code/Al-Wasl.Connect.git) and clone your fork
locally.

```text
$ git clone git@github.com/Call-for-Code/Al-Wasl.Connect
$ cd Al-Wasl.Connect
$ git remote add https://github.com/Call-for-Code/Al-Wasl.Connect.git
$ git fetch upstream
```
...
Al-Wasl.Connect does not yet have a style guide. If you're interested in creating one, you're on the right page.
...
Fixes: https://github.com/Call-for-Code/Al-Wasl.Connect/issues/1337
Refs: http://eslint.org/docs/rules/space-in-parens.html
```

If you are new to contributing to Al-Wasl.Connect, please try to do your best at
...
at least one Al-Wasl.Connect Collaborator. After that, as long as there are no
...
All  Al-Wasl.Connect contributors who choose to review and provide feedback on Pull
...
1. Does this change make sense for Al-Wasl.Connect?
2. Does this change make  Al-Wasl.Connect better, even if only incrementally?
...
Al-Wasl.Connect Collaborator landing the Pull Request but they can also be an
...
a particular change that makes Al-Wasl.Connect better, but the individual might just
not want to have anything to do with Al-Wasl.Connect ever again. The goal is not just
having good code.
...
Any Al-Wasl.Connect Collaborator (any GitHub user with commit rights in the
`Code-and-Response/Al-Wasl.Connect` repository) is authorized to approve any other contributor's
...
### Accept that there are different opinions about what belongs in Al-Wasl.Connect

Opinions on this vary, even among the members of the Technical Steering
Committee.

One general rule of thumb is that if Al-Wasl.Connect itself needs it (due to historic
or functional reasons), then it belongs in Al-Wasl.Connect.
...
make Al-Wasl.Connect better. These may or may not make sense to add, but as with all
...
Al-Wasl.Connect has always optimized for speed of execution. If a particular change
can be shown to make some part of Al-Wasl.Connect faster, it's quite likely to be
...
That said, performance is not the only factor to consider. Al-Wasl.Connect also
...
to make sure that it works on the platforms that Al-Wasl.Connect
